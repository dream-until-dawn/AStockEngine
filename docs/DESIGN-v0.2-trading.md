...
| 生效日 | 变更 | 状态 |
|---|---|---|
| 2023-08-28 | 由 0.1% 减半至 **0.05%** | 待核实 |
| 2008-09-19 | 由双边征收改为**单边**（仅卖出） | 待核实 |
| 2008-04-24 | 由 0.3% 降至 0.1% | 待核实 |
...
| 项 | 个股 | ETF |
|---|---|---|
| 佣金 | 万 2.5，最低 5 元（双向） | 同左 |
| 印花税 | 卖出 0.05%（2023-08-28 起） | **免征** |
| 过户费 | 双向 0.001% | **免征** |
...
| 项 | 标准 |
|---|---|
| **手工核算** | 个股与 ETF 各核算一笔完整交易的费用与持仓，与引擎输出**逐分位一致** |
| 时序 | 主板订单 T+1 成交、创业板可 T 日盘后成交，两条路径均有测试 |
| T+1 | 当日买入当日卖出必须被拒，理由为 `RejectInsufficientPosition` |
| 涨跌停 | 一字板买单被拒；涨停价买单被拒；理由结构化 |
| 公司行动 | 分红入账、送转增持、成本摊薄各有测试；缺记录时有警告 |
| 可复现 | 同配置两次运行**逐笔一致**（含费用与成交价） |
| 快照 | 含待撮合订单，恢复后成交结果一致 |
| 全整数 | 账本无任何 float |
