...
永续合约每 8 小时结算一次资金费率（多空互付），
**它对持仓成本的影响可能超过手续费**，而日线行情里没有这个数据。

当前状态：**未采集**。`instruments.attrs.funding_rate` 显式记为
`"not_collected"`，免得将来有人以为已经算进去了。

拿现在的数据回测永续合约，结果会**系统性偏乐观**（做多方在多数时间付费）。
补齐需要单独一张表（数据源 `/api/v5/public/funding-rate-history`），
届时在 1.2 之外新增 `funding` 表而非往 bar 里塞列 ——
结算频率（8h）与 bar 频率（1d）不同，塞进来就要先聚合，
而聚合方式本身是个会影响结果的选择。
...
回退到兜底值 252 —— 而加密的正确值是 365。
年化系数错 45% 是会改变结论的，**所以加密的年化必须走另一条路径**，
不能沿用日历。这一项在 CryptoMarket 落地时一并处理。
...
| 表 | 版本 | 说明 |
|---|---|---|
| `funding` | 未定 | 永续合约资金费率（8h 一次）。**缺它则加密回测系统性偏乐观**，见 1.5 |
