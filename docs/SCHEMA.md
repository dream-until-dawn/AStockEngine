...
> **已实现（v0.8）**：`trading.NotionalCents` / `QtyForCents` / `MulDiv`
> 一律走 `math/bits.Mul64` + `bits.Div64` 的 128 位中间量，
> 换算比由 `Instrument.NotionalRatio()` 先约分（A 股 → `1/10`、
> BTC → `1/1e16`）。
>
> 上线过程中这一类坑在三处各犯了一次，**每一处都不报错**：
> 定量按 A 股口径算出的张数被判成 0（168 条信号 → 0 笔成交）、
> 成交量上限乘积回绕（114 笔假拒单）、报告直接印定点数量
> （`1352000000 股`）。第三处的修法不是除以 scale 而是换一个量 ——
> 报开仓金额，因为**金额跨标的可加，定点数量不可加**。
> 详见 [DESIGN-v0.8-crypto.md](DESIGN-v0.8-crypto.md) §2。
