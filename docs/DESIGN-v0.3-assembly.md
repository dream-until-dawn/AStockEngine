...
`BpsSlippage.Apply` 里 `ref * bps / 10_000` 是**整数除法**，
每股滑点向下取整到厘，于是配置的 5 bp 实际只生效约 4.60 bp。
价格越低偏差越大：
...
| 2.400 元 | 1 厘 | 4.17 bp |
| 9.000 元 | 4 厘 | 4.44 bp |
| 100.000 元 | 50 厘 | 5.00 bp |
| 1297.400 元 | 648 厘 | 4.99 bp |

在 `buy_and_hold` 的隔离检验中实测：79.3 万元买入的滑点应为 396.36 元，
实际只有 364.90 元，**少算 7.9%**。

**方向是错的**：它让摩擦成本偏小、回测结果偏好，
而这个引擎的价值恰恰在于不美化结果。

这是 v0.2 就有的行为，不是本刀引入的。**没有在本刀修**，
因为改动会同时改掉上面刚建立的 v0.2 对照基线，
把「重构无行为变化」这条证据搅浑。建议单独一刀改（一行：向上取整），
并重新记录基线。
