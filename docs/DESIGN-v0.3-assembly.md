...
`fee.params.path` 只是文件放在哪，同一份费率换个目录不该改变指纹；
但费率**内容**必须进 —— 换一套费率结果就变了。
故取费率文件的规范化内容摘要入指纹，而不是路径字符串。
...
### 第二刀：Recorder + Metrics ✅（2026-08-29 完成）
...
**验收**：手工核算一个小样本（5 个交易日、3 笔来回）的
年化收益 / 夏普 / 最大回撤 / 胜率 / 盈亏比，与实现**逐位一致** ——
`internal/metrics/metrics_test.go` 里的数字全部取得能用计算器验：
日收益 +10/+10/−20/+12/−5%，样本方差 0.01898，最大回撤 20%，
三笔来回盈亏 +199,780 / −100,000 / 0 分，胜率 1/3，盈亏比 1.9978。
年化系数由日历数出（实测 20200102–20260828 段为 242.54，
20050104–20260828 段为 242.97），`TestAnnualisationUsesCalendarNot252`
钉住它换成 252 会同时抬高年化收益与年化波动。

#### 三个坑的落地情况

| 坑 | 落地 |
|---|---|
| 年化交易日不是 252 | `Calendar.TradingDaysPerYear` 按样本区间实测；报告把用到的系数印出来 |
| 一轮交易的定义 | `MatchRoundTrips` 按标的做 FIFO 逐层配对，成本与收入按份额比例分摊；送股份额零成本入队并标 `FromBonus`；未平仓不计入胜率但报出数量 |
| 基准只能用 ETF 代理 | 覆盖不到的时点直接跳过，报告印覆盖比例并加 ⚠。实测 2005 起的回测覆盖 3,466 / 5,260 = 65.89% |

#### 实跑出来的东西

`macd_cross`（2020–2026，285 只，等权 10 份）：

```
年化系数 242.54 交易日/年   无风险利率 0.00%
总收益 +11.06%   年化 +1.59%   年化波动 19.50%
夏普 0.081   索提诺 0.132   卡玛 0.058
最大回撤 27.34%   20220223 → 20231220（445 个交易日），20240930 恢复（188 个交易日）
完整轮次 890 轮（299 赢 / 591 输）  胜率 33.60%  盈亏比 1.966
年化换手 24.53 倍   摩擦占成交额 0.12%
对标 510300：策略 +11.06%  基准 +26.31%  超额 −15.25%
              Beta 0.483   Alpha +0.86%（年化）  信息比率 −0.094
```

**策略跑输了直接买沪深 300 ETF 15 个百分点。** 这正是基准该干的事 ——
+11.06% 单看是赚的，摆到基准旁边才知道它连被动持有都不如。
把 2005 起的全区间跑一遍更难看：策略 +14.83%，基准 +127.69%。

### 第三刀：结果指纹 ✅（2026-08-29 完成）
...
| 配置 | 输入指纹 | 输出指纹 |
|---|---|---|
| base | `2fcf4d3b8ba4e0ca` | `527df089655d1298` |
| base（第二次跑） | 相同 ✅ | 相同 ✅ |
| `slippage.bps` 5→6 | **变** ✅ | **变** ✅ |
| `recorder.level` summary→full | 相同 ✅ | 相同 ✅ |
| `name` 改掉 | 相同 ✅ | 相同 ✅ |
| `metrics.risk_free_ppm` 0→3% | 相同 ✅ | 相同 ✅ |

第 4 条（`data/` 复制到另一路径、mtime 全变，数据指纹不变）
由 `TestDataFingerprintSurvivesCopy` 覆盖。

另加两项设计验收：

- `TestEveryResultAffectingFieldEntersFingerprint` 逐个确认 10 个
  影响结果的字段都真的进了指纹。**漏掉任何一个都比没有指纹更糟** ——
  它会给出「同配置」的假象，而结果其实不同。
- 快照往返现在比对**指纹**而不只是账本三个数：
  账本相同只说明结果相同，指纹相同才说明逐笔成交都相同。
  实测第 800 步快照恢复后跑完，指纹与全程完全一致。

#### 三个实现决定

**输出指纹在引擎内滚动计算，不是事后从 Recorder 的成交流算。**
`recorder.level = none` 根本不留成交；若指纹依赖记录，换个记录级别指纹就变了 ——
而记录级别只决定留多少东西给人看，不决定算什么。代价是每笔成交多一次
哈希写入（1,787 笔无可测影响）。

**滚动哈希的状态进快照。** 不进的话，从快照恢复后跑出来的指纹会缺掉
快照之前的全部成交 —— 而 C6 的实盘模式**每天都从快照恢复**，
那样指纹永远对不上全程运行，「同配置同结果」也就无从验证。
`sha256` 实现了 `encoding.BinaryMarshaler`，直接序列化即可。

**数据指纹哈希内容，不哈希 mtime。** 缓存用 `(相对路径, 大小, mtime)` 作键
只是为了跳过重算，缓存失效时重算出的结果与原来一致。
只覆盖 parquet：CSV 镜像是派生物、`_sync_state.json` 是 ETL 进度，
都不影响回测结果。首次全量哈希 335 MB 约 5 秒，之后走缓存。

## 9.4 第二刀的设计取舍

**Recorder 由引擎持有，不由驱动方持有。** 单步驱动、批量海选、实盘增量
三种模式共用同一个核心（C4），记录逻辑写在驱动里就要写三遍。

**`Summary` 级必须留全部成交。** 胜率、盈亏比、换手率都从成交流算，
只留每步汇总数就算不了。成交笔数比时点数少一个量级（1,787 vs 1,614），
留下来不构成负担。

**`Full` 级必须拷贝明细。** 引擎给的切片下一步就被复用，
留引用等于留一堆会变的数据。`TestFullKeepsDetailAndCopies` 钉住这条。

**基准载入但不入标的池。** `LoadDataSet` 把基准标的一并载入，
`Assemble` 再把它裁掉 —— 否则策略会把基准当成可交易标的，
超额收益就成了自己跟自己比。验证：加入基准前后 `macd_cross`
的成交笔数与期末权益完全一致。

**下行波动的分母用全部样本数**，不是仅下行样本数。这是索提诺比率的通行口径；
...
| 策略 | 成交 | 拒单 | 期末权益 | 收益 | 峰值回撤 | 费用 | 滑点 |
|---|---|---|---|---|---|---|---|
| `buy_and_hold` | 10 | 0 | 1,311,552.75 | +31.16% | 26.81% | 258.11（0.03%） | 496.24（0.05%） |
| `ma_cross` | 1,438 | 5,722 | 630,052.71 | −36.99% | 49.40% | 93,387.58（9.34%） | 71,161.84（7.12%） |
| `macd_cross` | 1,787 | 738 | 1,110,600.57 | +11.06% | 27.34% | 115,644.39（11.56%） | 88,857.96（8.89%） |
