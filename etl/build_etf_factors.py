...
#
# 合并的判定阈值取 1.25（价格上涨 25%），依据是**任何 ETF 的单日涨幅上限是 20%**
# （跟踪创业板/科创板指数者），超过它就不可能是市场波动，只能是折算。
# 首版取 1/1.5=0.667 过于保守，把 0.667~1.001 这段留成了盲区：
# 159845 于 2023-03-01 的 1:1.34 合并（比值 0.7477）就落在里面被跳过。
#
# 相反方向（价格下跌）不需要这么紧的阈值：分红与拆分都会压低价格，
# 两者在数值上无法截然区分，但拆分分支会尝试取整、失败则回落到原值，
# 与分红分支的结果一致，故阈值取多少对数值影响很小。
_MERGE_RISE_LIMIT = 1.25
_MERGE_THRESHOLD = 1.0 / _MERGE_RISE_LIMIT
