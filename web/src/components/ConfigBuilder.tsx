        ...
        {isCrypto && (
          <>
            <p className="note">
              ...
              各自独立记账、独立爆仓。红利税不适用（永续没有分红送配）。
            </p>
            <p className="note">
              <strong>杠杆只有在仓位基准选 notional 时才会放大敞口。</strong>
              其余三个基准（cost / equity / initial）都是本金口径 ——
              杠杆只会让保证金占用变少、闲置现金变多，
              而<strong>敞口一分不变</strong>，同时因为每个仓位的保证金更薄，
              更容易被强平。实测同一份配置把杠杆从 1 调到 20：
              单边成交额只从 165,318 变到 164,988，强平却从 0 次涨到 139 次。
              维持保证金率则是直接有效的：0.10% 时强平 6 次，10% 时 52 次。
            </p>
          </>
        )}
      </Section>
